...
function OngletSuiviTop({ annee, semaine, criterion }) {
  ...
  const exportExcel = () => {
    const rows = displayPdvs.map((p, i) => ({
      'Rang': i + 1, 'PDV': p.nom, [getMetricLabel(criterion)]: getMetricValue(p, criterion), 'Quartier': p.quartier || '-',
...
function OngletPareto({ annee, semaine, criterion }) {
  ...
  const metricFort = fortImpact.reduce((s, p) => s + getMetricValue(p, criterion), 0);
  const metricFaible = faibleImpact.reduce((s, p) => s + getMetricValue(p, criterion), 0);
  const totalMetricLabel = getMetricLabel(criterion);

  const exportExcel = () => {
    const rows = paretoList.map(p => ({
      'Rang': p.rang, 'PDV': p.nom, 'Zone': p.zone || '-', 'Superviseur': p.superviseur || '-',
      [totalMetricLabel]: getMetricValue(p, criterion),
      [`% ${totalMetricLabel}`]: p.pct_ca.toFixed(2),
      'Cumul %': p.cumul_pct.toFixed(2),
    ...
    <div>
      <div className="grid-3 mb-24">
        ...
        <div className="card" style={{ borderLeft: '3px solid #FF6900' }}>
          <div style={{ fontSize: 12, color: '#8a8a9a', marginBottom: 6 }}>{totalMetricLabel} Total Semaine</div>
          <div style={{ fontSize: 24, fontWeight: 800, color: '#FF6900' }}>{formatCA(totalMetric)}</div>
