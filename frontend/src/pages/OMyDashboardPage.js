...
function TabProgression({ annee, criterion }) {
  ...
  // Tous les PDVs avec progression calculée selon criterion
  ...
  const getHistMetric = (h) => {
    ...
    return h.ca || 0; // montant_transaction par défaut
  };

  // Réguliers : actifs (metric > 0) dans les 5 mois selon criterion
  const pdvsReguliers = allPDVs.filter(p => {
    const hist = p.historique_mensuel || [];
    const moisActifs = hist.filter(h => getHistMetric(h) > 0);
    return moisActifs.length >= 5;
  });

  // Toujours Top 10 : recalculer le rang par mois selon criterion
  const pdvsToujours10 = (() => {
    ...
    // Calculer le rang de chaque PDV dans chaque mois
    const nbFoisTop10 = {};
    ...
    return allPDVs.filter(p => (nbFoisTop10[p.pdv_id] || 0) >= nbMois && nbMois > 0);
  })();

  // Meilleur/Pire mois du réseau selon criterion
    ...
    <div>
      ...
      {activeFilter && (
        <div style={{ marginBottom: 8, display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 12, color: '#FF6900', fontWeight: 600 }}>
            Filtre: {activeFilter === 'reguliers' ? '✅ Réguliers' : activeFilter === 'top10' ? '🏆 Toujours Top 10' : activeFilter} ({allSortedPDVs.length} PDVs)
          </span>
          <button onClick={() => setActiveFilter(null)} style={{ fontSize: 11, background: 'none', border: 'none', color: '#8a8a9a', cursor: 'pointer' }}>✕ Effacer</button>
        </div>
      )}
      ...
      <div className="pdv-pagination">
        <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>
          Page {page} / {totalPages} · {allSortedPDVs.length} PDVs
        </span>
...
// ============ MAIN COMPONENT ============
export default // ─── Multi-select dropdown pour mois comparaison ─────────────────────────────
...
// ─── Tab Comparaison Multi-Mois OMY ──────────────────────────────────────────
const MOIS_OPTIONS = [
...
function TabComparaisonOMY() {
  ...
  const exportExcel = () => {
    ...
    const ws = XLSX.utils.json_to_sheet(rows);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Comparaison OMY');
  ...
  return (
    <div>
      {/* Header */}
      <div style={{ marginBottom:20 }}>
        <div style={{ fontSize:16, fontWeight:900, color:'#fff', marginBottom:6 }}>📊 Comparaison Multi-Mois OMY</div>
        <div style={{ fontSize:12, color:'#64748b' }}>Identifiez les PDVs OMY en baisse et planifiez les actions de relance</div>
      </div>
