...
@app.get("/resync-commissions")
async def resync_commissions():
    """Resynchroniser toutes les commissions depuis les performances"""
    from app.core.database import SessionLocal
    ...
    stats = db.query(CommissionEntry.pdv_type, func.count(CommissionEntry.id)).group_by(CommissionEntry.pdv_type).all()
    db.close()
    return {"total": inseres, "par_type": {str(t): c for t, c in stats}}
